...
const Hobby = ({ onSubmit, onBack, defaultValues, isFirst }) => {
  const { control, register, handleSubmit, formState: { errors } } = useForm({
    defaultValues: defaultValues || { hobbies: [{ hobby: '' }] }
  });
            ...
            <div className='w-full border-2 border-transparent rounded-4xl bg-cyan-50 p-6'>
              ...
              {fields.map((item, idx) => (
                <div key={item.id} className="flex items-center mb-4">
                  <input
                    type="text"
                    placeholder="Ex: Painting, Football, Reading"
                    {...register(`hobbies.${idx}.hobby`, { required: true })}
                    className="create-job-input flex-1"
                    aria-invalid={errors.hobbies?.[idx]?.hobby ? "true" : "false"}
                  />
                  {fields.length > 1 && (
                    <button
                      type="button"
